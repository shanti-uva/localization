...
class ContextsControllerTest < ActionController::TestCase
  ...
  test "should create context" do
    assert_difference('Context.count') do
      post :create, :context => { :document_id => @context.document_id, :key => @context.key, :message_id => @context.message_id }
    end
  ...
  test "should update context" do
    put :update, :id => @context, :context => { :document_id => @context.document_id, :key => @context.key, :message_id => @context.message_id }
    assert_redirected_to context_path(assigns(:context))
  end
